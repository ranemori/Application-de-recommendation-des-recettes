...
class RecommendationService:
    ...
    def _effective_cuisines(self, user: User) -> set:
        """The set of cuisines that should count as 'this user's taste', combining:
    ...
    def for_user(self, user: User, n: int = 12) -> List[RecommendationOut]:
        ...
        freshness) on top of the already-trained ALS model
        """
    ...
    def _content_scores(self, recipe: Recipe) -> dict:
        ...
        for cand in candidates:
            ...
            # IDF-weighted Jaccard: sharing one rare, distinctive ingredient counts far more than sharing several generic pantry staples
    ...
    def _pad_with_same_cuisine(self, recipe: Recipe, top: list, n: int) -> list:
        ...
        with same-cuisine popular recipes rather than an unrelated grab-bag of random recipes"""
    ...
    def _popularity(self, n: int = 12, user: Optional[User] = None) -> List[RecommendationOut]:
        ...
        pool by rating
        that lets whichever single cuisine happens to
        have the highest-rated (or most numerous) dishes crowd out all
        the others entirely
        Instead we round-robin: best-rated pick from cuisine 1, then cuisine 2,
